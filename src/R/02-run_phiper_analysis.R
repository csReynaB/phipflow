...
for (cmp_idx in seq_along(comparisons)) {
  ...
  ranks_combined <- c(RANK_COLS, "peptide_id")
  ...
  res_filtered <- res %>% 
    dplyr::filter(m_eff > DELTA_MIN_M_EFF, p_perm < 0.05)
    ...
    if (nrow(res_plot) == 0) {
      ...
      forest_plot <- NULL
        ...
        if (nrow(plot_top) == 0) {
            ...
            write.csv(plot_top, 
                file = file.path(out_dir, "DELTA_framework", paste0("delta_table_m_eff_", DELTA_MIN_M_EFF, "TOP_", taxon, "_sort_by_maxDelta-Stouffer.csv") ), row.names = FALSE, quote = TRUE)
  ...
  tax_cols <- intersect(c(RANK_COLS), names(peplib))
